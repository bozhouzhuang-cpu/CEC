...
def main():
    """Main function to run displacement computation for all year combinations."""
    ...
    # Auto-detect years from filenames
    import glob
    import re
    
    # Find all PCD files matching the pattern
    pattern = os.path.join(base_path, "upsampled_subset_ptCloud_site1_*.pcd")
    ...
    for file_path in pcd_files:
        ...
        match = re.search(r'upsampled_subset_ptCloud_site1_(\d{4})\.pcd', filename)
        if match:
    ...
    if not years:
        print("No PCD files found matching the pattern!")
        return
    ...
    for i, year1 in enumerate(years):
        for year2 in years[i+1:]:  # Only compare with later years
            total_combinations += 1
    
    print(f"Processing {total_combinations} year combinations...")
    ...
    for i, year1 in enumerate(years):
        for year2 in years[i+1:]:  # Only compare with later years
            ...
            print(f"\n{'='*60}")
            print(f"Combination {combination_count}/{total_combinations}: {year1} vs {year2}")
            print(f"{'='*60}")
            
            # File paths
            file1 = os.path.join(base_path, f"upsampled_subset_ptCloud_site1_{year1}.pcd")
            file2 = os.path.join(base_path, f"upsampled_subset_ptCloud_site1_{year2}.pcd")
            ...
            try:
                results = processor.compute_displacement_vectors(
                    file1, file2, 
                    grid_size=3,
                    output_suffix=f'_{year1}to{year2}'
                )
                
                if results:
                    print(f"✓ Successfully completed {year1} vs {year2}")
                else:
                    print(f"✗ Failed to process {year1} vs {year2}")
                    
            except Exception as e:
                print(f"✗ Error processing {year1} vs {year2}: {e}")
                continue
