...
Uses ALL points (no sampling) for maximum spatial coverage.

Usage:
  python compute_displacement_fullgrid_nosampling.py
"""
...
class LidarDisplacementFullGrid:
	...
	def load_point_cloud_full(self, file_path):
		"""Load ALL points from PCD file - no sampling."""
		if not os.path.exists(file_path):
			raise FileNotFoundError(f"PCD file not found: {file_path}")
		print(f"Loading {os.path.basename(file_path)} (all points)...")
	...
	def compute_displacement_vectors(self, file1_path, file2_path, grid_size, output_suffix):
		print("=== LiDAR Displacement Vector Computation (All Points) ===")
		print(f"\nProcessing: {os.path.basename(file1_path)} -> {os.path.basename(file2_path)}")
		print(f"Grid size: {grid_size}")
		start_time = time.time()
		
		# Load ALL points (no sampling)
		pts1, count1 = self.load_point_cloud_full(file1_path)
		pts2, count2 = self.load_point_cloud_full(file2_path)
		...
		out_file = os.path.join(out_dir, f"displacement_results_fullgrid_allpoints_{output_suffix}.npz")
		np.savez(out_file,
...
def main():
	processor = LidarDisplacementFullGrid(use_gpu=True)
	...
	if res:
		print("=== Full-grid Processing (All Points) Complete ===")
	else:
		print("=== Full-grid Processing (All Points) Failed ===")
