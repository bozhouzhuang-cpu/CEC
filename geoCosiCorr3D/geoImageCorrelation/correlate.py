...
from geoCosiCorr3D.geoCore.core_correlation import RawCorrelation, FreqCorrelator, SpatialCorrelator, NCCCorrelator, WaveletNCCCorrelator, DTCWTNCCCorrelator
...
class Correlate(RawCorrelation):
    ...
    def run_corr_engine(self, base_array, target_array):
        ...
        elif self.corr_engine.correlator_name == CORRELATION.NCC_CORRELATOR:
            return NCCCorrelator.run_correlator(base_array=base_array,
                                                target_array=target_array,
                                                window_size=self.corr_engine.corr_params.window_size,
                                                step=self.corr_engine.corr_params.step,
                                                search_range=self.corr_engine.corr_params.search_range)

        elif self.corr_engine.correlator_name == CORRELATION.WAVELET_NCC_CORRELATOR:
            p = self.corr_engine.corr_params
            return WaveletNCCCorrelator.run_correlator(
                base_array=base_array, target_array=target_array,
                window_size=p.window_size, step=p.step, search_range=p.search_range,
                wavelet=getattr(p, 'wavelet', 'db2'),
                levels=getattr(p, 'levels', 3),
                min_tpl_var=getattr(p, 'min_tpl_var', 1e-4),
                min_peak=getattr(p, 'min_peak', 0.5),
            )

        elif self.corr_engine.correlator_name == CORRELATION.DTCWT_NCC_CORRELATOR:
            p = self.corr_engine.corr_params
            return DTCWTNCCCorrelator.run_correlator(
                base_array=base_array, target_array=target_array,
                window_size=p.window_size, step=p.step, search_range=p.search_range,
                levels=getattr(p, 'levels', 4),
                min_tpl_var=getattr(p, 'min_tpl_var', 1e-4),
                min_peak=getattr(p, 'min_peak', 0.5),
            )
