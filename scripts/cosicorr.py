...
def correlate_func(args):
    ...
    if args.method == C.CORR_METHODS.NCC_CORR.value:
        corr_config = {
            "correlator_name": C.CORR_METHODS.NCC_CORR.value,
            "correlator_params": {
                "window_size": args.window_size,
                "step": args.step,
                "grid": args.grid,
                "search_range": args.search_range or [6, 6]  # sensible default
            }
        }
    ...
    elif args.method == C.CORR_METHODS.WAVELET_NCC.value:
        corr_config = {
            "correlator_name": C.CORR_METHODS.WAVELET_NCC.value,
            "correlator_params": {
                "window_size": args.window_size,
                "step": args.step,
                "grid": args.grid,
                "search_range": args.search_range or [6, 6],  # same semantics as spatial/NCC
                "wavelet": args.wavelet,
                "levels": args.levels,
                "min_tpl_var": args.min_tpl_var,
                "min_peak": args.min_peak,
            }
        }

    elif args.method == C.CORR_METHODS.DTCWT_NCC.value:
        corr_config = {
            "correlator_name": C.CORR_METHODS.DTCWT_NCC.value,
            "correlator_params": {
                "window_size": args.window_size,
                "step": args.step,
                "grid": args.grid,
                "search_range": args.search_range or [6, 6],
                "levels": args.dtcwt_levels,
                "min_tpl_var": args.min_tpl_var,
                "min_peak": args.min_peak,
            }
        }
            ...
            for base_image, target_image, base_band, target_band in zip(base_images, target_images, base_bands,
                ...
                try:
                    correlate_func(argparse.Namespace(base_image=base_image,
                                                  ...
                                                  nb_iters=args.nb_iters,
                                                  search_range=args.search_range,
                                                  # NEW: pass wavelet params through
                                                  wavelet=getattr(args, "wavelet", "db2"),
                                                  levels=getattr(args, "levels", 3),
                                                  min_tpl_var=getattr(args, "min_tpl_var", 1e-4),
                                                  min_peak=getattr(args, "min_peak", 0.5)
                                                  ))
                except Exception as e:
...
def correlate_subparser(subparsers):
    ...
    correlate_parser.add_argument("--method", type=str,
                                  choices=[C.CORR_METHODS.FREQUENCY_CORR.value, C.CORR_METHODS.SPATIAL_CORR.value,C.CORR_METHODS.NCC_CORR.value, C.CORR_METHODS.WAVELET_NCC.value, C.CORR_METHODS.DTCWT_NCC.value],
    ...
    # Wavelet-NCC arguments (new group)
    wave_group = correlate_parser.add_argument_group("Wavelet-NCC arguments")
    wave_group.add_argument("--wavelet", type=str, default="db2",
                            help="PyWavelets mother wavelet (db2, sym4, coif1, ...)")
    wave_group.add_argument("--levels",  type=int, default=3,
                            help="SWT levels (2–4 typical)")
    wave_group.add_argument("--min_tpl_var", type=float, default=1e-4,
                            help="Reject low-texture templates")
    wave_group.add_argument("--min_peak",    type=float, default=0.5,
                            help="Min fused NCC peak to accept")

    # Add DTCWT argument group
    dtcwt_group = correlate_parser.add_argument_group("DTCWT-NCC arguments")
    dtcwt_group.add_argument("--dtcwt_levels", type=int, default=4,
                            help="DTCWT decomposition levels (2-5 typical)")
...
def batch_correlate_subparser(subparsers):
    ...
    batch_correlate_parser.add_argument("--method", type=str, choices=[C.CORR_METHODS.FREQUENCY_CORR.value,
                                                                       ...
                                                                       C.CORR_METHODS.NCC_CORR.value,
                                                                       C.CORR_METHODS.WAVELET_NCC.value],
    ...
    # Specific arguments for wavelet-ncc method
    wave_group = batch_correlate_parser.add_argument_group("Wavelet-NCC arguments")
    wave_group.add_argument("--wavelet", type=str, default="db2")
    wave_group.add_argument("--levels",  type=int, default=3)
    wave_group.add_argument("--min_tpl_var", type=float, default=1e-4)
    wave_group.add_argument("--min_peak",    type=float, default=0.5)
